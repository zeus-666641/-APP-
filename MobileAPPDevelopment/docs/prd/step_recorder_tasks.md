...
| ID | 任务 | 模块 | 预估 | 前置 | 验收 |
|---|---|---|---|---|---|
| T5.1 | log 模型 + log_service（CRUD+4维过滤+分组+清理+mock） | services | 1.5h | T0.2 | 日志读写 + 单测（68 项） |
| T5.2 | logs_view（列表+4维过滤+搜索+分组切换）+ log_detail_view（独立路由+时间轴） | views | 2h | T5.1 | 列表+详情+步骤时间轴展开（44 项测试） |
| T5.3 | log_card 组件（卡片+左下角删除+二次确认） | views/components | 0.5h | T5.2 | 卡片样式 + 删除二次确认 |
| T5.4 | settings_view 加"日志管理"区域（自动清理开关+保留条数 1-9999） | views | 0.5h | T5.1 | 自动清理可配置 |
| T5.5 | routes.py 接入 /logs 与 /logs/{id} 独立路由 | config | 0.5h | T5.2 | 路由跳转生效 |
| T5.6 | home_view stats tab 旁加 logs 入口 | views | 0.5h | T5.5 | tab 内双按钮可跳转 |

**M5 合计：5.5h**

> 实际完成情况：M5 共完成 6 个任务（T5.1-T5.6），测试 431 项全绿（其中 M5 模块新增 112 项）。
...
- M3 触发器：7h
- M4 统计：4.5h
- M5 日志：5.5h（原估 3.5h，实际 +2h 用于 settings 集成和路由配置）
- M6 设置：2h
- M7 执行引擎：6.5h
- M8 打包：1h
- **合计：58h**（约 14.5 个工作日）
